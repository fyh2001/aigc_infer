"""
Qwen Image Edit inference using native HuggingFace Diffusers.
For benchmarking comparison with the SGLang version (run_qwen_image_edit.py).

Supports:
  - Qwen/Qwen-Image-Edit-2511  (QwenImageEditPlusPipeline)
  - Qwen/Qwen-Image-Edit-2509  (QwenImageEditPlusPipeline)
  - Qwen/Qwen-Image-Edit       (QwenImageEditPipeline)

Usage examples:
  # Basic image editing
  python run_diffusers_image_edit.py \
      --image-path input.jpg \
      --prompt "Turn it into a watercolor painting."

  # Multi-image fusion (2511 feature)
  python run_diffusers_image_edit.py \
      --image-path img1.jpg img2.jpg \
      --prompt "Two people standing together in a park."
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run Qwen Image Edit with native Diffusers (baseline)",
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Path(s) to input image(s). Multiple images for multi-image fusion.",
    )
    parser.add_argument(
        "--prompt",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=" ",
        help="Negative prompt (default: single space, as recommended by official docs)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    gen_group.add_argument("--steps", type=int, default=40, help="Number of denoising steps")
    gen_group.add_argument("--cfg-scale", type=float, default=4.0, help="true_cfg_scale for prompt adherence")
    gen_group.add_argument("--guidance-scale", type=float, default=1.0, help="guidance_scale (typically 1.0 for edit models)")
    gen_group.add_argument("--seed", type=int, default=42, help="Random seed")
    gen_group.add_argument("--num-outputs", type=int, default=1, help="Number of images to generate per prompt")

    perf_group = parser.add_argument_group("Performance options")
    perf_group.add_argument(
        "--cpu-offload",
        action="store_true",
        help="Enable model CPU offload to save GPU VRAM",
    )
    perf_group.add_argument(
        "--device",
        type=str,
    ...
    perf_group.add_argument(
        "--torch-compile",
        action="store_true",
        help="Apply torch.compile to the transformer for faster inference",
    )
    perf_group.add_argument(
        "--attention-backend",
        type=str,
        default=None,
        help="Attention backend (e.g. flash_attn, sage_attn)",
    )
...
def _load_images(image_paths: list[str]) -> list[Image.Image]:
    ...
    for p in image_paths:
        ...
        logger.info("  Loaded image: %s (%dx%d)", p, img.width, img.height)
        images.append(img)
    return images
...
def _load_pipeline(args, device):
    ...
    is_base_edit = "2509" not in model_path_lower and "2511" not in model_path_lower

    if is_base_edit:
        from diffusers import QwenImageEditPipeline as PipelineCls
    else:
        from diffusers import QwenImageEditPlusPipeline as PipelineCls

    dtype = torch.bfloat16 if device != "cpu" else torch.float32
    logger.info("Pipeline class: %s", PipelineCls.__name__)
    logger.info("dtype: %s, device: %s", dtype, device)

    pipeline = PipelineCls.from_pretrained(args.model_path, torch_dtype=dtype)
...
def _run_generate(pipeline, images, args, device, run_label="Generate"):
    ...
    for i, img in enumerate(result_images):
        if args.output_file_name:
            ...
            fname = f"edit_{timestamp}_s{args.seed}_{i}.png"
...
def main():
    ...
    logger.info("Image(s): %s", args.image_path)
    ...
    if not args.profile:
        _, elapsed, _ = _run_generate(pipeline, images, args, device, run_label="Run")
        logger.info("-" * 60)
    ...
    for wi in range(args.profile_warmup):
        _run_generate(pipeline, images, args, device, run_label=f"Warmup {wi + 1}/{args.profile_warmup}")
    ...
    for ri in range(args.profile_repeat):
        trace_json = trace_dir / f"diffusers_edit_trace_run{ri}.json"
        trace_gz = trace_dir / f"diffusers_edit_trace_run{ri}.json.gz"
        ...
        with profile(
            ...
            with record_function("diffusers_image_edit_generate"):
                _, wall_time, _ = _run_generate(
                    pipeline, images, args, device,
